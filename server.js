...
async function init() {
  ...
  // On s'assure que le compte Superviseure "shiphra" existe TOUJOURS avec le bon
  // mot de passe, même si une base plus ancienne contient déjà d'autres comptes
  // (ex: un ancien compte "patronne" créé par une version précédente).
  ...
  if (existing.rows.length === 0) {
    await pool.query(
      ...
      ['shiphra', 'Superviseure', 'superviseur', hash, salt]
    );
    console.log('Compte Superviseure créé -> identifiant: shiphra.');
  } else {
    await pool.query(
      `UPDATE users SET password_hash=$1, password_salt=$2, role='superviseur', active=TRUE, must_change_password=FALSE WHERE username='shiphra'`,
      [hash, salt]
    );
    console.log('Compte Superviseure "shiphra" vérifié / mot de passe resynchronisé.');
  }
